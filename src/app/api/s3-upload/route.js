...
// Public URL for the uploaded object (works for public-read buckets)
const region = process.env.AWS_REGION;
const bucket = process.env.S3_BUCKET_NAME;
const publicUrl = `${url.replace(/\/$/, '')}/${uniqueKey}`;

// Return the URL, fields, and the generated key to the client
return NextResponse.json({ url, fields, key: uniqueKey, publicUrl, bucket, region });
} catch (error) {
